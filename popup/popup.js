...
const DEFAULT_SETTINGS = {
  ...
  processEtc: false,
  enabled: true,
  destinations: { NOTEBOOKLM: '', CLAUDE: '', CHATGPT: '', ETC: '' }
};
...
const destNLM         = document.getElementById('destNLM');
const destCDL         = document.getElementById('destCDL');
const destGPT         = document.getElementById('destGPT');
...
function applySettings(settings) {
  ...
  destNLM.value          = settings.destinations.NOTEBOOKLM;
  destCDL.value          = settings.destinations.CLAUDE;
  destGPT.value          = settings.destinations.CHATGPT;
}
...
async function saveSettings() {
  ...
  const settings = {
    ...
    destinations: {
      NOTEBOOKLM: destNLM.value.trim(),
      CLAUDE:     destCDL.value.trim(),
      CHATGPT:    destGPT.value.trim(),
      ETC:        ''
    }
  };
...
function renderLogs(logs) {
  ...
  logList.innerHTML = logs.map(entry => {
    const statusClass = entry.status === 'moved'
      ? 'moved'
      : entry.status === 'error'
        ? 'error'
        : entry.source.toLowerCase();

    const badge = entry.status === 'error'
      ? 'ERR'
      : entry.status === 'moved'
        ? '→'
        : getBadgeLabel(entry.source);

    const filename = entry.newFilePath
      ? entry.newFilePath.split(/[/\\]/).pop()
      : entry.originalFilename;

    const tooltip = entry.status === 'error'
      ? (entry.errorMessage ?? '')
      : (entry.newFilePath ?? '');
